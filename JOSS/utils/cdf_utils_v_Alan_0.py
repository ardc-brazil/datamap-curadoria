...
def get_day_data(all_data, export_date, variables_info):
    ...
    # reindex the dataframe with the day index and fill the missing values with the missing value or NaN
...
def get_ri_rd80(data, drop_size, drop_class):

    # calculate the "constant" based on the interval (it can change!)
    c = (np.pi/6.) * (3.6/(10.**3.))
    inv_interval = 1 / (0.005 * data["Interval"].values)

    # extract drop sizes and diameters as NumPy arrays: times x raindrop classes
    drop_sizes = data[drop_size].values
    diameters = np.tile(np.array(drop_class), drop_sizes.shape[0]).reshape(drop_sizes.shape[0],drop_sizes.shape[1])
...
def get_liq_water_rd80(data, drop_size, drop_class, fall_vell):
...
def get_z_rd80(data, drop_size, drop_class, fall_vell):
...
def get_zdb(z_data):
    return [10 * math.log10(z) if z > 0 else 0 for z in z_data]


def get_ek_rd80(data, drop_size, drop_class, fall_vell):

    # calculate the "constant" based on the interval (it can change!)
    c = (np.pi/12.)*(1/0.005)*(1/10**6)
...
def get_n_d_rd80(data, drop_size, delta_diam, fall_vell):
    ...
    t = data["Interval"].values

    n_d = []
    for i in range(len(t)):
        n = drop_sizes[i,:] / ((0.005*t[i])*(vel[i,:]*delta_diam[i,:]))
        n_d.append(n)
