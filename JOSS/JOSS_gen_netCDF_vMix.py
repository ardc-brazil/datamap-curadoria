...
if args.standard:
    ...
    #Essa busca depende do nome do arquivo, então se mudar o nome do arquivo, tem que mudar aqui também
    files = [
        path_input_data.joinpath(file)
        for days in [d1, d0, d2]
        for file in os.listdir(path_input_data)
        if file.find(days) != -1
        #if file.endswith(EXT) and any(dt_str in file for dt_str in [d0, d1, d2])
    ]
#    count = 0
#    for file in os.listdir(path_input_data):
#        print(file)
#        #check if exit d0 or d1 or d2 in files string
#        for days in [d1, d0, d2]:
#            print(days)
#            if file.find(days) != -1:
#                print("The short string is in the long string.")
#                files=path_input_data.joinpath(file)
#                count+=1
#            else:
#                print("The short string is not in the long string.")
