...
def get_filter_ni(data, drop_col, min_ni=10):
...
def get_ri_rd80(data, drop_col, drop_diam):
    ...
    drop_sizes = data[drop_col].values
...
def get_liq_water_rd80(data, drop_col, drop_diam, fall_vell):
...
def get_z_rd80(data, drop_col, drop_diam, fall_vell):
...
def get_zdb(z_data):
    zdbz = np.where((z_data > 0), 10 * np.log10(z_data), z_data)
    return zdbz


def get_ek_rd80(data, drop_col, drop_diam, fall_vell):
...
def get_d_max(data,drop_col, drop_diam):
...
def get_d_min(data,drop_col, drop_diam):
...
def get_n_d_rd80(data, drop_col, delta_diam, fall_vell):
    ...
    n_d = drop_sizes / ((0.005 * t[:, np.newaxis]) * (vel * delta_diam))
