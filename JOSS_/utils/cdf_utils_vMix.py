...
def get_ri(data, drop_size, drop_class):

    rain_rate = []

    # for i in range(len(data)):
    #     row = data.iloc[i]
    #     r = (np.pi/6.) * (3.6/(10.**3.)) * (1/(0.005*row['Interval'])) * (np.sum(np.array(row[drop_size]) * np.array(drop_class)**3))
    #     rain_rate.append(r)
    
    c = (np.pi/6.) * (3.6/(10.**3.))
    rain_rate = [    c * (1/(0.005*row['Interval'])) * sum((row[n] * pow(diam, 3))
            for n, diam in zip(drop_size, drop_class)
        )
...
def get_liq_water(data, drop_size, drop_class, fall_vell):
    
    #(np.pi/6.) * (1/(0.005*a['Interval'])) * (np.sum((np.array(row[drop_size])/fall_vell) * np.array(drop_class)**3))
    c = (np.pi/6.)
    liq_water = [ c * (1/(0.005*row['Interval'])) * sum((row[n]/vel) * pow(diam, 3)
            for n, diam, vel in zip(drop_size, drop_class, fall_vell)
        )
        for _, row in data.iterrows()
    ]
...
def get_z(data, drop_size, drop_class, fall_vell):

    z = [ (1/(0.005*row['Interval'])) * sum((row[n]/vel) * pow(diam, 6)
            for n, diam, vel in zip(drop_size, drop_class, fall_vell)
        )
        for _, row in data.iterrows()
    ]  
